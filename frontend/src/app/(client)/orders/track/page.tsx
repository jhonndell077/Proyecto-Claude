      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-4">
        ...
        <div className="card">
          <div className="flex items-center gap-3">
            ...
            {order.business?.phone && <a href={`tel:${order.business.phone}`} className="ml-auto p-2 bg-brand-50 rounded-xl text-brand-500"><Phone size={18} /></a>}
          </div>
        </div>
        ...
        {order.driver && (
          <div className="card bg-brand-50 border border-brand-100">
            <div className="flex items-center gap-3">
              ...
              <div><p className="font-semibold text-sm">Repartidor asignado</p><p className="text-xs text-gray-600">{order.driver.user?.name}</p></div>
              {order.driver.user?.phone && <a href={`tel:${order.driver.user.phone}`} className="ml-auto p-2 bg-white rounded-xl text-brand-500 border border-brand-200"><Phone size={16} /></a>}
            </div>
          </div>
        )}
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            ...
            <div className="border-t pt-2 mt-2 space-y-1">
              {order.deliveryFee > 0 && <div className="flex justify-between text-sm text-gray-500"><span>Delivery</span><span>{formatCurrency(order.deliveryFee)}</span></div>}
              <div className="flex justify-between font-bold"><span>Total</span><span className="text-brand-500">{formatCurrency(order.total)}</span></div>
            </div>
          </div>
        </div>
...
export default function OrderTrackPage() {
  return <Suspense><OrderTrackContent /></Suspense>;
}
